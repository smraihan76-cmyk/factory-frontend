...
function Dashboard({ currentUser, onLogout, onUpdateUser }) {
  ...
  const submitWholesalerRate = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/wholesalers/${selectedWholesalerForRate.id}/rates`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const quickActions = [
    ...
    { icon: <Package size={24} className="text-indigo-700" />, bg: 'bg-indigo-100', label: 'পাইকারি হিসাব', onClick: () => {} },
              ...
              <>
                ...
                {wholesalerRates.length > 0 && (
                  <div className="flex flex-col gap-2.5 mb-4">
                    {wholesalerRates.map((r) => (
                      <div key={r.id} className="bg-white rounded-2xl shadow-sm p-3.5 flex items-center justify-between border-l-4 border-gray-200">
                ...
                <div className="bg-white rounded-2xl shadow-md p-4 border-2 border-violet-200">
                  ...
                  >
                    ...
                    {wholesalerRateSubmitting ? 'সেভ হচ্ছে...' : 'সেভ করুন'}
                  </button>
                </div>
              </>
            )}
