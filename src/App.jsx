...
function Dashboard({ currentUser, onLogout, onUpdateUser }) {
  ...
  // ফোনের ব্যাক বাটন চাপলে যেন পুরো অ্যাপ বন্ধ না হয়ে, খোলা থাকা মডাল/পেজটাই বন্ধ হয়
  const closeAllModals = () => {
    ...
    setDetailView(null);
    setCashMemoStaff(null);
  };

  const anyModalOpen =
    showAbsentModal || showAddForm || showAttendanceModal || showBalanceDetail || showDutyForm ||
    showEditProfile || showEmployeeModal || showExpenseForm || showExpenseReport || showFundChoice ||
    showKarigorHisab || showMachineForm || showNotificationHistory || showNotifications ||
    showPartnerList || showPartnerLogPage || showPartnerResetConfirm || showPaymentResetConfirm ||
    showProductForm || showProfileMenu || showResetConfirm || showUserManagement || showWeeklyPicker ||
    !!selectedPartner || !!partnerTxnForm || !!viewingReactorsTxn || !!detailView || !!cashMemoStaff;
                    ...
                    {partnerTransactions.length === 0 ? (
                      ...
                      <div className="flex flex-col gap-2.5">
                        {partnerTransactions.map((t) => (
                          <div
                            key={t.id}
                            className="bg-white rounded-2xl shadow-sm border border-gray-200 p-3.5"
                          >
      ...
      <div className={`w-full sm:max-w-sm bg-stone-100 min-h-screen relative pb-20 ${(cashMemoStaff || showExpenseReport) ? 'print:hidden' : ''}`}>
        ...
        {cashMemoStaff && (
          <div className="fixed inset-0 bg-black/50 flex items-end justify-center z-50 print:bg-white print:static print:block">
            <div className="w-full sm:max-w-sm bg-white rounded-t-3xl print:rounded-none p-6 max-h-[85vh] print:max-h-none overflow-y-auto print:overflow-visible">
