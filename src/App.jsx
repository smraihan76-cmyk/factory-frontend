...
function Dashboard({ currentUser, onLogout }) {
  ...
  const presentCount = attendanceToday.filter((s) => s.status === 'present' || s.status === 'on_break').length;
  ...
  const quickActions = [
    ...
    { icon: <Server size={24} className="text-red-800" />, bg: 'bg-red-100', label: 'মেশিন যোগ করুন', onClick: () => { setShowMachineForm(true); fetchMachines(); fetchSyncInterval(); } },
