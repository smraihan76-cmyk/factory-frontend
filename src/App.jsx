...
function Dashboard({ currentUser, onLogout }) {
  ...
  const submitPartnerTxn = async () => {
    ...
    try {
      ...
      if (data.status === 'ok') {
        ...
        openPartnerDetail(selectedPartner);
      } else {
  ...
  const quickActions = [
    { icon: <RefreshCw size={24} className="text-rose-700" />, bg: 'bg-rose-100', label: 'পার্টনার হিসাব', onClick: openPartnerList },
  ...
  return (
    <div className="min-h-screen bg-stone-100 flex justify-center">
      <div className={`w-full max-w-sm bg-stone-100 min-h-screen relative pb-20 ${(cashMemoStaff || showExpenseReport) ? 'print:hidden' : ''}`}>
        ...
        {/* পার্টনার হিসাব — পার্টনার লিস্ট */}
        {showPartnerList && (
          <div className="fixed inset-0 bg-black/50 flex items-end justify-center z-50">
            <div className="w-full max-w-sm bg-white rounded-t-3xl p-6 max-h-[85vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-5">
                <h2 className="text-lg font-bold text-gray-900">পার্টনার হিসাব</h2>
                <button onClick={() => setShowPartnerList(false)} className="text-gray-400">
                  <X size={22} />
                </button>
              </div>
              {partners.length === 0 ? (
                <p className="text-sm text-gray-500 text-center py-8">এখনো কোনো পার্টনার যোগ করা হয়নি</p>
              ) : (
                <div className="flex flex-col gap-3">
                  {partners.map((p) => (
                    <button
                      key={p.id}
                      onClick={() => { setShowPartnerList(false); openPartnerDetail(p); }}
                      className="text-left bg-white rounded-2xl shadow-md border-2 border-gray-200 p-4 flex items-center justify-between gap-3 active:opacity-80"
                    >
                      <div>
                        <p className="font-semibold text-gray-900 text-sm">{p.name}</p>
        ...
        {/* পার্টনার হিসাব — নতুন/এডিট এন্ট্রি ফর্ম */}
        {partnerTxnForm && (
          <div className="fixed inset-0 bg-black/50 flex items-end justify-center z-50">
            <div className="w-full max-w-sm bg-white rounded-t-3xl p-6">
              <div className="flex items-center justify-between mb-5">
