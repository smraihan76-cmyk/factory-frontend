...
function Dashboard({ currentUser, onLogout }) {
  ...
  const stats = [
    {
      icon: <User size={22} className="text-amber-600" />, bg: 'bg-amber-100', dot: 'bg-amber-500',
      value: `${staffList.length}`, label: 'মোট এমপ্লয়ি', onClick: () => setShowEmployeeModal(true),
      cardBg: 'bg-gradient-to-br from-amber-50 to-orange-50',
      wave: 'M0,20 Q25,10 50,18 T100,15 V30 H0 Z', waveColor: '#fed7aa'
    },
    {
      icon: <CheckCircle2 size={22} className="text-emerald-700" />, bg: 'bg-emerald-100', dot: 'bg-emerald-600',
      value: `${presentCount}`, label: 'মোট উপস্থিত', onClick: openAttendanceModal,
      cardBg: 'bg-gradient-to-br from-emerald-50 to-teal-50',
      wave: 'M0,18 Q25,25 50,15 T100,20 V30 H0 Z', waveColor: '#a7f3d0'
    },
    {
      icon: <MapPin size={22} className="text-orange-700" />, bg: 'bg-orange-100', dot: 'bg-orange-600',
      value: `${absentCount}`, label: 'মোট অনুপস্থিত', onClick: openAbsentModal,
      cardBg: 'bg-gradient-to-br from-rose-50 to-orange-50',
      wave: 'M0,22 Q25,12 50,20 T100,16 V30 H0 Z', waveColor: '#fecdd3'
    },
  ];

  const quickActions = [
    { icon: <RefreshCw size={24} className="text-rose-700" />, bg: 'bg-rose-100', label: 'পার্টনার হিসাব', onClick: () => {} },
    ...
    { icon: <CheckCircle2 size={24} className="text-yellow-700" />, bg: 'bg-yellow-100', label: 'মজুরী', onClick: () => {} },
  ...
  return (
    <div className="min-h-screen bg-stone-100 flex justify-center">
      <div className={`w-full max-w-sm bg-stone-100 min-h-screen relative pb-20 ${cashMemoStaff ? 'print:hidden' : ''}`}>
        ...
        {/* Header */}
        <div className="relative bg-gradient-to-br from-red-950 via-red-900 to-black rounded-b-3xl px-6 pt-8 pb-16 text-white overflow-hidden">
          <div className="absolute inset-0 opacity-20" style={{ backgroundImage: 'radial-gradient(circle at 85% 20%, rgba(217,180,80,0.5), transparent 45%)' }} />
          ...
          <div className="absolute top-8 right-6 flex items-center gap-3">
            <div className="relative w-10 h-10 rounded-full bg-white/10 flex items-center justify-center">
              <Bell size={18} />
              <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-red-500 border border-red-950" />
            </div>
            <button
              onClick={() => setShowProfileMenu(true)}
              className="w-11 h-11 rounded-full bg-amber-500 text-red-950 flex items-center justify-center font-bold"
            >
        ...
        {/* Balance Card */}
        <div className="mx-4 -mt-10 bg-white rounded-2xl shadow-md p-5">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-900 font-bold text-lg mt-0.5">
                  {balanceHidden
                    ? <span className="text-sm font-medium text-gray-500">দেখতে "ব্যালেন্স দেখুন" চাপুন</span>
                    : `৳ ${staffList.reduce((sum, s) => sum + computeStaffDue(s, paymentsSummaryAll, salarySummaryAll), 0).toFixed(2)}`}
                </p>
              </div>
            </div>
            {balanceTrend && (
              <div className={`shrink-0 rounded-xl px-2.5 py-1.5 text-center ${balanceTrend.direction === 'up' ? 'bg-emerald-50' : 'bg-red-50'}`}>
        ...
        <div className="flex gap-3 px-4 mt-3">
          {stats.map((s, i) => (
            <div key={i} onClick={s.onClick} className={`relative flex-1 rounded-2xl p-3.5 shadow-sm active:opacity-80 cursor-pointer overflow-hidden ${s.cardBg}`}>
              <svg className="absolute bottom-0 left-0 w-full h-8 opacity-60" viewBox="0 0 100 30" preserveAspectRatio="none">
                <path d={s.wave} fill={s.waveColor} />
              </svg>
              ...
              <p className="relative text-lg font-bold text-gray-900 mt-2 leading-tight">{s.value}</p>
              <p className="relative text-xs text-gray-500 mt-0.5 leading-snug">{s.label}</p>
            </div>
          ))}
        </div>
        ...
        <div className="px-4 mt-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs font-semibold text-gray-400 flex items-center gap-1">সব অপশন দেখুন →</span>
          </div>
          <div className="grid grid-cols-4 gap-2.5">
            {quickActions.map((a, i) => (
              <button
                key={i}
                onClick={a.onClick}
                className={`rounded-2xl p-2.5 shadow-sm active:opacity-80 flex flex-col items-start gap-2 ${a.bg}`}
              >
                ...
                <div className="w-full flex items-start justify-between gap-0.5">
                  <span className="text-[10px] font-semibold text-gray-800 text-left leading-tight">{a.label}</span>
                  <ChevronRight size={11} className="text-gray-400 shrink-0 mt-0.5" />
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="px-4 mt-6">
          ...
          <button
            ...
            className="w-full bg-white rounded-2xl shadow-sm p-4 flex items-center gap-3 active:opacity-80 text-left"
          >
        ...
        {/* Bottom nav */}
        <div className="fixed bottom-0 w-full max-w-sm bg-white border-t border-gray-200 flex items-center justify-around py-2.5">
          {navItems.map((n, i) =>
            n.active ? (
              <button key={i} className="flex items-center gap-2 bg-red-950 text-white rounded-full px-5 py-2.5">
                {n.icon}
                <span className="text-xs font-medium">{n.label}</span>
              </button>
            ) : (
              <button key={i} className="relative flex flex-col items-center gap-1 px-4 text-gray-400">
                {n.icon}
                {n.label === 'অ্যালার্ট' && <span className="absolute top-0 right-2 w-1.5 h-1.5 rounded-full bg-red-500" />}
                <span className="text-xs font-medium">{n.label}</span>
              </button>
            )
          )}
        ...
        {showProfileMenu && (
          <div className="fixed inset-0 bg-black/50 flex items-end justify-center z-50">
            <div className="w-full max-w-sm bg-white rounded-t-3xl p-6">
              ...
              <div className="bg-white rounded-2xl shadow-md p-4 border-l-4 border-red-900 mb-4">
        ...
        {showUserManagement && (
          <div className="fixed inset-0 bg-black/50 flex items-end justify-center z-50">
            <div className="w-full max-w-sm bg-white rounded-t-3xl p-6 max-h-[85vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-5">
                <h2 className="text-lg font-bold text-gray-900">নতুন এডমিন/মডারেটর যোগ করুন</h2>
                <button onClick={() => setShowUserManagement(false)} className="text-gray-400">
                  <X size={22} />
                </button>
              </div>
              ...
              <form onSubmit={handleAddUser} className="space-y-4">
                ...
                >
                  ...
                  {userSubmitting ? 'সেভ হচ্ছে...' : 'যোগ করুন'}
                </button>
              </form>
                ...
                <div className="mt-6">
                  ...
                  <div className="flex flex-col gap-3">
                    {users.map((u) => (
                      <div
                        key={u.id}
                        className={`bg-white rounded-2xl shadow-md p-4 flex items-center justify-between gap-3 border-l-4 ${u.role === 'admin' ? 'border-red-900' : 'border-amber-500'}`}
                      >
                        <div className="min-w-0">
                          <p className="font-semibold text-gray-900 text-sm">{u.name}</p>
                          <p className="text-xs text-gray-500 mt-0.5">{u.phone} · {u.role}</p>
                        </div>
                        {u.phone !== '01775515571' && (
                          <button
